...
public class ItemUUBow extends ItemToolEMCC // ItemBow
{
	...
	public void onPlayerStoppedUsing(ItemStack is, World w, EntityPlayer ep, int itemInUseCount)
	{
		...
		if (flag || ep.inventory.hasItem(Items.arrow))
		{
			...
			if (f == 1F)
			{
				ea.setIsCritical(true);
			}

			int k = getInfusionLevel(is, ToolInfusion.SHARPNESS);
			if (k > 0)
			{
				ea.setDamage(ea.getDamage() + (double) k * 0.5D + 0.5D);
			}

			int l = getInfusionLevel(is, ToolInfusion.KNOCKBACK);
			if (l > 0)
			{
				ea.setKnockbackStrength(l);
			}

			if (hasInfusion(is, ToolInfusion.FIRE))
			{
				ea.setFire(100);
			}
			...
			w.playSoundAtEntity(ep, "random.bow", 1F, 1F / (Item.itemRand.nextFloat() * 0.4F + 1.2F) + f * 0.5F);
			...
			{
				ep.inventory.consumeInventoryItem(Items.arrow);
			}
